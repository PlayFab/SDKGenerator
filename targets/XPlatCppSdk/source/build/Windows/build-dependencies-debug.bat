...
rmdir "%externalDir%/deps-debug" /s /q
...
pushd "%externalDir%/openssl-build-debug"
...
pushd "winbuild"
nmake /f Makefile.vc mode=static VC=15 DEBUG=yes WITH_SSL=static WITH_ZLIB=static MACHINE=x64
if /I "%ERRORLEVEL%" neq "0" (
...
rem - copy headers and binaries to deps
set curlBuildDir=builds/libcurl-vc15-x64-debug-static-ssl-static-zlib-static-ipv6-sspi/
xcopy "%curlBuildDir%include/*.*" "%deps%include/" /E /R /F /Y /H
xcopy "%curlBuildDir%lib/libcurl_a_debug.lib" "%deps%lib/" /R /F /Y /H
popd

echo --- Building jsoncpp ---
pushd "%externalDir%/jsoncpp-build"
msbuild.exe lib_json.sln /nologo /t:Build /p:Configuration="Debug" /p:Platform=x64
if /I "%ERRORLEVEL%" neq "0" (
...
xcopy "x64/Debug/lib_json.lib" "%deps%lib/" /R /F /Y /H
xcopy "x64/Debug/lib_json.pdb" "%deps%lib/" /R /F /Y /H
popd

rem - copy the deps folder to build
xcopy "%externalDir%/deps" "%externalDir%/deps-debug" /I /E /R /F /Y /H
popd
