...
exports.main = function (sourceDir, outputDir, targetName, pluginCache, config) {
    // Generate Plugin Manager
    var pluginManagerLocals = {};
    pluginManagerLocals.pluginInfos = [];
    pluginManagerLocals.configSettings = [];

    // Initialize collection of PluginInfo objects
    for (var pluginId in pluginCache) {
        var pluginDescriptor = pluginCache[pluginId];
        // Find the right target in plugin's descriptor (e.g. csharp)
        for (var i = 0; i < pluginDescriptor.targets.length; i++) {
            if (pluginDescriptor.targets[i].name == targetName && pluginDescriptor.targets[i].code) {
                // Create PluginInfo object and add it to the collection in locals for template
                var pluginInfo = {
                    id: pluginCache[pluginId].id,
                    code: {
                        headerInclude: pluginDescriptor.targets[i].code.headerInclude,
                        callCreate: pluginDescriptor.targets[i].code.callCreate
                    }
                };

                pluginManagerLocals.pluginInfos.push(pluginInfo);
            }
        }
    }

    // Initialize collection of ConfigSettings objects
    for (var i = 0; i < config.plugins.length; i++) {
        var settings = config.plugins[i].configuration.settings;
        if (settings) {
            for (var j = 0; j < settings.length; j++) {
                pluginManagerLocals.configSettings.push(settings[j]);
            }
        }
    }

    var pluginManagerTemplate = getCompiledTemplate(path.resolve(sourceDir, "templates/common-client-lib/PluginManager.cs.ejs"));
    writeFile(path.resolve(outputDir, "source/common-client-lib/PluginManager.cs"), pluginManagerTemplate(pluginManagerLocals));

    // Generate project files
    if (config.pluginSubtypePath == "PlayFabClientSDK")
        generateProject(sourceDir, outputDir, pluginCache, "Client", "ENABLE_PLAYFABENTITY_API");
    if (config.pluginSubtypePath == "PlayFabServerSDK")
        generateProject(sourceDir, outputDir, pluginCache, "Server", ";DISABLE_PLAYFABCLIENT_API;ENABLE_PLAYFABENTITY_API");
    if (config.pluginSubtypePath == "PlayFabSDK")
        generateProject(sourceDir, outputDir, pluginCache, "All", "ENABLE_PLAYFABADMIN_API;ENABLE_PLAYFABSERVER_API;ENABLE_PLAYFABMATCHMAKER_API;ENABLE_PLAYFABENTITY_API");
...
function generateProject(sourceDir, outputDir, pluginCache, libname, extraDefines) {
    var projLocals = {
    ...
    var projTemplate = getCompiledTemplate(path.resolve(sourceDir, "templates/PlayFabSDK.csproj.ejs"));
    writeFile(path.resolve(outputDir, "PlayFabSDK.csproj"), projTemplate(projLocals));
}
