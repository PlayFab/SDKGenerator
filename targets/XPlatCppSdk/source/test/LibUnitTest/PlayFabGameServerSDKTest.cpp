    ...
    namespace GSDK
    {
        ...
        public:
            TEST_METHOD_CLEANUP(SetTestInstanceToNull)
            {
                ...
                PlayFabGameServerInternal::testConfiguration.reset();
                PlayFabGameServerInternal::m_instance.reset();
            }
            ...
            TEST_METHOD(ConfigAllSetInitializesFine)
            {
                ...
                ports["port1"] = "1111";
                ports["port2"] = "2222";
                PlayFabGameServerInternal::testConfiguration = std::make_unique<TestConfig>(heartbeatEndpoint, serverId, logFolder, sharedContentFolder, certFolder, gameCerts, titleId, buildId, region, metadata, ports);
                PlayFabGameServerSDK::start();
                const std::unordered_map<std::string, std::string> &config = PlayFabGameServerSDK::getConfigSettings();
                Assert::AreEqual(heartbeatEndpoint, config.at("heartbeatEndpoint"), L"Ensuring heartbeat endpoint was set.");
                Assert::AreEqual(serverId, config.at("serverId"), L"Ensuring server id was set.");
                ...
                Assert::AreEqual(logFolder, PlayFabGameServerSDK::getLogsDirectory(), L"Ensuring  get log folder works.");
                Assert::AreEqual(sharedContentFolder, PlayFabGameServerSDK::getSharedContentDirectory(), L"Ensuring  get shared content folder works.");
            ...
            {
                PlayFabGameServerInternal::testConfiguration = std::make_unique<TestConfig>("heartbeatEndpoint", "serverId", "", "sharedContentFolder");
                PlayFabGameServerSDK::start();
                Assert::IsTrue("" == PlayFabGameServerSDK::getLogsDirectory(), L"Ensuring  get log folder works.");
            }

            TEST_METHOD(SharedContentFolderNotSetInitializesFine)
            {
                PlayFabGameServerInternal::testConfiguration = std::make_unique<TestConfig>("heartbeatEndpoint", "serverId", "logFolder", "");
                PlayFabGameServerSDK::start();
                Assert::IsTrue("" == PlayFabGameServerSDK::getSharedContentDirectory(), L"Ensuring  get shared Content folder works.");
            }
            ...
            TEST_METHOD(HeartbeatEndpointNotSetFailsToInitialize)
            {
                try
                {
                    PlayFabGameServerInternal::testConfiguration = std::make_unique<TestConfig>("", "serverId", "logFolder", "sharedContentFolder");
                    PlayFabGameServerSDK::start();
            ...
            TEST_METHOD(ServerIdNotSetFailsToInitialize)
            {
                try
                {
                    PlayFabGameServerInternal::testConfiguration = std::make_unique<TestConfig>("heartbeatEndpoint", "", "logFolder", "sharedContentFolder");
                    PlayFabGameServerSDK::start();
            ...
            {
                PlayFabGameServerInternal::testConfiguration = std::make_unique<TestConfig>("heartbeatEndpoint", "serverId", "logFolder", "sharedContentFolder");
                PlayFabGameServerSDK::start();

                // Test Initial State
                Json::Value jsonHeartbeatRequest = parseJson(PlayFabGameServerInternal::m_instance->encodeHeartbeatRequest());
                ...
                PlayFabGameServerSDK::registerHealthCallback([]() -> bool { return false; });
                ...
                PlayFabGameServerSDK::updateConnectedPlayers(players);
                ...
                PlayFabGameServerInternal::m_instance->setState(GameState::StandingBy);

                jsonHeartbeatRequest = parseJson(PlayFabGameServerInternal::m_instance->encodeHeartbeatRequest());
            ...
            TEST_METHOD(DecodeAgentResponseJsonCorrectly)
            {
                ...
                PlayFabGameServerSDK::registerMaintenanceCallback([&maintenanceTime](tm t) -> void { maintenanceTime = _mkgmtime(&t); });
                ...
                PlayFabGameServerInternal::m_instance->decodeHeartbeatResponse(responseJson);
                ...
                Assert::IsTrue(PlayFabGameServerInternal::m_instance->m_heartbeatRequest.m_currentGameState == GameState::Active, L"Verify state was changed.");
            }
            ...
            TEST_METHOD(ReturnInitialPlayerListFromJson)
            {
                ...
                Assert::AreEqual((size_t)0, PlayFabGameServerSDK::getInitialPlayers().size(), L"InitialPlayer list is empty before allocation.");
                ...
                // Test heartbeat response handled correctly
                const std::vector<std::string> &players = PlayFabGameServerSDK::getInitialPlayers();
            ...
            TEST_METHOD(AgentOperationStateChangesHandledCorrectly)
            {
                ...
                PlayFabGameServerSDK::registerShutdownCallback([&shutdownCalled]() -> void { shutdownCalled = true; });

                // Call ReadyForPlayers in another thread so we don't block this one
                std::future<bool> readyForPlayersThread = std::async(std::launch::async, []() -> bool { return PlayFabGameServerSDK::readyForPlayers(); });
                ...
                Assert::IsTrue(PlayFabGameServerInternal::m_instance->m_heartbeatRequest.m_currentGameState == GameState::Active, L"Verify state stayed as Active.");
