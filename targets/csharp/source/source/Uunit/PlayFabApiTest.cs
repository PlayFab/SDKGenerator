...
namespace PlayFab.UUnit
{
    public class PlayFabApiTest : UUnitTestCase
    {
        ...
        {
            var playFabTransport = PluginManager.GetPlugin(PluginContract.PlayFab_Transport) as ITransportPlugin;
            var playFabHeaders = new Dictionary<string, string>() { { "X-Authorization", PlayFabSettings.ClientSessionTicket } };
            var playFabUrlPath = "/Client/GetUserData";

            // Create multiple custom plugins
            const int pluginNumber = 4;
            var customTransports = new ITransportPlugin[pluginNumber];
            var wasDoPostCalled = new bool[pluginNumber];
            var APIs = new Func<Task<PlayFabResult<GetUserDataResult>>>[pluginNumber];
            for (int i = 0; i < pluginNumber; i++)
            {
                int j = i; // Make a copy of an index so that it's captured correctly in the lambdas below (not as a ref)
                var customTransport = CreateCustomTransportPlugin(
                // DoPost action
                async (string urlPath, object request, Dictionary<string, string> headers) =>
                {
                    wasDoPostCalled[j] = true;

                    // Call PlayFab's implementation from inside a custom plugin to act as a real implementation
                    return await playFabTransport.DoPost(urlPath, request, headers);
                });

                customTransports[i] = customTransport;
                wasDoPostCalled[i] = false;

                // Create a custom API that use each plugin
                APIs[i] = async () =>
                {
                    var request = new GetUserDataRequest();
                    var transport = (ITransportPlugin)PluginManager.GetPlugin(PluginContract.PlayFab_Transport, "Plugin_" + j.ToString());
                    var httpResult = await transport.DoPost(playFabUrlPath, request, playFabHeaders);
            ...
            try
            {
                // Set multiple custom plugins with the same contract
                for (int i = 0; i < pluginNumber; i++)
                {
                    PluginManager.SetPlugin(customTransports[i], PluginContract.PlayFab_Transport, "Plugin_" + i.ToString());
                }

                // Call the APIs
                int totalNumberOfCalls = 0;
                var resultTasks = new ConcurrentBag<Task<PlayFabResult<GetUserDataResult>>>();
                Parallel.ForEach(APIs, api =>
                {
                    // Each API uses its specific transport plugin
                    var task = api();
                    ContinueWithContext(task, testContext, null, true, "GetUserData call failed", false);
                ...
                for (int i = 0; i < pluginNumber; i++)
                {
                    testContext.True(wasDoPostCalled[i]);
                    testContext.IsNull(resultTasksArray[i].Result.Error);
                }
                testContext.EndTest(UUnitFinishState.PASSED, null);
            }
            catch (Exception e)
