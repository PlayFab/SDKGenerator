...
public class PlayFabHTTP {
    ...
    private static Object doPostPrivate(String url, Object request, String authType, String authKey) throws Exception {
        ...
        try {
            ...
            return GeneratePfError(httpCode, PlayFabErrorCode.ServiceUnavailable, "Failed to post to server: " + url, null, Integer.MAX_VALUE);
        }
        ...
        if(httpCode != 200 || responseString == null || responseString.isEmpty()) {
            if(responseString == null || responseString.isEmpty() || httpCode == 404 )
                return GeneratePfError(httpCode, PlayFabErrorCode.ServiceUnavailable, "Empty server response", null, Integer.MAX_VALUE);

            PlayFabJsonError errorResult = null;
            try {
                errorResult = gson.fromJson(responseString, PlayFabJsonError.class);
            } catch(Exception e) {
                return GeneratePfError(httpCode, PlayFabErrorCode.JsonParseError, "Server response not proper json :" + responseString, null, errorResult.retryAfterSeconds);
            }
